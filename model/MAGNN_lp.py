...
class MAGNN_metapath_specific(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.rnn_type == 'gru':
            ...
            edata = edata.reshape(edata.shape[0], edata.shape[1], edata.shape[2] // 2, 2)  # 123,3,32,2
            final_r_vec = torch.zeros([edata.shape[1], self.out_dim // 2, 2], device=edata.device)  # 3,32,2
            final_r_vec[-1, :, 0] = 1
            # print("edata", edata.shape[0], edata.shape[1])
            # print("etypes",self.etypes)
            # print("final_r_vec", len(final_r_vec), len(final_r_vec[0]), len(final_r_vec[0][0]))
        ...
        # print(len(ret[target_idx]),len(ret[target_idx][0]),len(ret[target_idx][0][0]))
        # print(self.etypes,ret[target_idx])
        if self.use_minibatch:
            # print(len(ret),len(edata),len(edge_metapath_indices),target_idx)
...
class GCN_layer(nn.Module):
    def __init__(self, dim, mp_ls=None):
        ...
        self.gcn1 = nn.Parameter(torch.zeros([dim, 256]), requires_grad=True)
        self.gcn2 = nn.Parameter(torch.zeros([512, 2]), requires_grad=True)
    ...
    def forward(self, x, layer):
        ...
        x_dot = torch.matmul(x, x.T)
        x_norm = torch.norm(x, dim=1, keepdim=True)
        cos_sim = x_dot / (torch.matmul(x_norm, x_norm.T) + 1e-8)
        adj = F.softmax(cos_sim, dim=-1)
        if layer == 0:
...
class MAGNN_lp_layer(nn.Module):
    def __init__(self,
        ...
        hidden_dim = 128  # Define the hidden dimension of the MLP
        self.classifier = GCN_layer(dim=in_dim * num_heads)
        self.mlp_user = MLP(in_dim * num_heads, hidden_dim, out_dim)
        self.mlp_item = MLP(in_dim * num_heads, hidden_dim, out_dim)
    ...
    def forward(self, inputs):
        ...
        gcn_user = self.classifier(h_user, 0)
        gcn_item = self.classifier(h_item, 0)
        x = torch.cat([gcn_user, gcn_item], dim=1)
        x_out = self.classifier(x, 1)
