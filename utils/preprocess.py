...
def get_networkx_graph(neighbor_pairs, type_mask, ctr_ntype,num_mi):
    ...
    for metapaths in neighbor_pairs:
        ...
        G = nx.MultiDiGraph()
        G.add_nodes_from(range(ctr_ntype*num_mi,len(indices)+ctr_ntype*num_mi))
        for (src, dst), paths in sorted_metapaths:
            for _ in range(len(paths)):
                G.add_edge(idx_mapping[src]+ctr_ntype*num_mi, idx_mapping[dst]+ctr_ntype*num_mi)
                edge_count += 1
        G_list.append(G)
    return G_list
