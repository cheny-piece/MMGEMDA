...
def parse_adjlist(adjlist, edge_metapath_indices, samples=None):
    ...
    for row, indices in zip(adjlist, edge_metapath_indices):
        # print(len(row),len(indices))
        row_parsed = list(map(int, row.split(' ')))
        # print(len(row_parsed))
    ...
    mapping = {map_from: map_to for map_to, map_from in enumerate(sorted(nodes))}#{'':1,'':2}
...
def parse_adjlist_MD(adjlist, edge_metapath_indices, samples=None, exclude=None, offset=None, mode=None):
    edges = []
    nodes = set()
    result_indices = []
    for row, indices in zip(adjlist, edge_metapath_indices):
...
def parse_minibatch_MD(adjlists_ua, edge_metapath_indices_list_ua, user_artist_batch, device, samples=None,
    ...
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_ua, edge_metapath_indices_list_ua)):
        for adjlist, indices, use_mask in zip(adjlists, edge_metapath_indices_list, use_masks[mode]):
            # print(len(adjlists),len(adjlist))
            if use_mask:
